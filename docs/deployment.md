...
下载校验二进制、创建系统用户与数据目录、生成运行配置、安装并启动 systemd 服务，一步到位：
...
- **真实客户端 IP**：程序自动按 `CF-Connecting-IP` → `X-Forwarded-For` → `RemoteAddr` 解析真实来源 IP，
  **无需任何配置**即可适配 Cloudflare / Caddy / Nginx / 直连（登录失败限流按真实 IP 封禁，且绝不封禁
  本地/私有 IP，避免误伤全体）。**前提**：本服务只经反代访问、不直接裸奔公网（绑定 `127.0.0.1`/私有网或
  防火墙限制），确保转发头由可信反代设置。
...
- 透传真实客户端 IP 到 `X-Forwarded-For`（程序取最左侧最初客户端；Cloudflare 用 `CF-Connecting-IP`）。
- 保证回源为同源路径（`/api/*` 与 `/` 同域），cookie 与 CSRF 才能正常工作。

### WebSocket（WSS）

agent 经 `wss://<host>/api/agent/ws` 主动连接：
...
agent 可经 `POST /api/agent/pull` 兜底轮询（无需长连接）：
...
- **Cloudflare**：在 443 前置即可，默认支持 WebSocket 透传；回源指向上述反代或直连 `MGATE_HTTP_ADDR`。
  经 Cloudflare 时真实 IP 在 `CF-Connecting-IP`，程序会自动采用，无需额外配置。
...
- HTTPS Pull 兜底：`POST /api/agent/pull` 与 WS 共用鉴权与 lease，互不重复投递。
