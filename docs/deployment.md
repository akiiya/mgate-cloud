...
- **真实客户端 IP**：程序默认信任来自本地回环/私有网段的反代转发头，故置于本机/Docker 内的
  Caddy/Nginx 之后**无需额外配置**即可识别真实 IP（登录失败限流按真实 IP 封禁，避免误伤全体）。
  反代若位于**非私有地址**（如直接源站于 Cloudflare、自建公网反代），用
  `MGATE_TRUSTED_PROXIES=<CIDR,...>` 指明其网段；程序**直接裸奔公网**则设 `MGATE_TRUSTED_PROXIES=none`。
...
- 透传真实客户端 IP 到 `X-Forwarded-For`（程序取最右侧非可信代理地址作为真实客户端）。
...
- **Cloudflare**：在 443 前置即可，默认支持 WebSocket 透传；回源指向上述反代或直连 `MGATE_HTTP_ADDR`。
  经 Cloudflare 时真实 IP 在 `CF-Connecting-IP`：若 Cloudflare 回源到本机/私有网的反代再到程序，
  默认即生效；若 Cloudflare **直接回源到公网暴露的程序**，需 `MGATE_TRUSTED_PROXIES=<Cloudflare 网段>`。
