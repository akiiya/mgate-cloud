...
本文描述白名单命令队列：动作白名单、参数规则、消息协议、命令状态机、
结果存储与截断策略。
...
>
> **enabled 设备无论在线与否都可创建命令**，离线命令进入队列等待 HTTPS Pull
...
在 `agent.hello/heartbeat/status` 与 `server.hello/pong/error` 之上新增：
...
- 命令取消是 **cloud 侧标记**，**不**通知 agent，**不保证**终止 agent 已开始的本地执行。
