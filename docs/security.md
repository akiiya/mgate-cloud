...
- 封禁按**真实客户端 IP**进行：IP 经代理感知策略解析（见下「可信代理策略」）。置于本地/私有网反代
  之后无需额外配置即生效；公网非私有反代需用 `MGATE_TRUSTED_PROXIES` 指明，否则会把反代后的所有人
  归并到同一 IP 而相互误伤。
...
- **可信代理策略（代理感知的真实客户端 IP）**：
  - **仅当“直接对端”是可信代理时**才采信其转发头（`CF-Connecting-IP` 优先，其次 `X-Forwarded-For`
    从右向左取首个非可信代理地址）；否则一律以对端 `RemoteAddr` 为准——公网直连客户端伪造
    `X-Forwarded-For` 无效。
  - **默认即信任本地回环 + 私有/链路本地网段**（典型反代位置），因此置于 Caddy/Nginx/Docker 之后
    **开箱即用**，能正确识别真实客户端 IP（对登录失败限流至关重要——按真实 IP 封禁，不会把反代后
    所有人归并到同一 IP 而误伤）。
  - 反代位于**非私有地址**（自建公网反代、特定 CDN 段）时，用 `MGATE_TRUSTED_PROXIES`（逗号分隔
    CIDR / IP）追加可信网段。
  - 程序**直接暴露公网、无任何反代**时，设 `MGATE_TRUSTED_PROXIES=none` 关闭一切转发头信任。
  - `MGATE_TRUST_PROXY_HEADERS=true` 为兼容旧行为的“无条件信任任意对端转发头”，一般无需开启。
